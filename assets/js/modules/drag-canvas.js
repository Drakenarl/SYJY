...
export function initDragCanvas() {
  ...
  /* ------------------------------------------------------------------
     CORRECTIF MOBILE — important.

     Le viewport occupe 100% de la hauteur de l'écran et capturait le
     pointeur (touch-action: none + setPointerCapture). Résultat : sur
     téléphone, glisser le doigt sur le hero déplaçait le canvas au lieu
     de faire défiler la page. L'utilisateur restait bloqué en haut.

     Sur écran tactile, le canvas devient donc un décor fixe : on ne
     branche aucun gestionnaire et le geste vertical revient au scroll.
     ------------------------------------------------------------------ */
